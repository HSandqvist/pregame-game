...
export default {
  ...
  created: function () {
    ...
    socket.on("topAnswerUpdate", function ({ topAnswer, maxVotes }) {
      ...
      document.getElementById("topAnswer").innerText = `Top Answer: ${topAnswer}`; //ändrar direkt i html element med det i
  ...
  methods: {
    ...
    submitAnswer: function (answer) {
      ...
      console.log("Answer sent to server:", { pollId: this.pollId, answer: answer });
    ...
    loadQuestions(questionCount) {
      ...
      for (let i = 0; i < questionCount; i++) {
       ...
       if (randomQuestion && !this.isQuestionDuplicate(questions, randomQuestion)) {
          questions.push(randomQuestion);
        } else {
          i--; // If the question was a duplicate, try again
        }
      }
      // Store the random questions to be displayed
